...
\textit{application program interface} (API). In some languages,
such as C, depending on how the vendor provides the library a binary
...
the dependencies of a software package. A common solution is to take a
sequence-based scheme and attach meaning to the positions in order to
measure degree of compatibility. That way the library authors take
responsibility for noting changes in their software -- as is
appropriate, because offloading this reponsibility to the users
increases cost disproportionally.
...
handle obtained by calling functions from \codeinline{tempfile} , we can
...
\codeinline{Makefile} found in the repository has the following targets:

\begin{itemize}
\item \codeinline{make test} runs the test suite.
\item \codeinline{make lint} runs the linter \codeinline{flake8} (or the one
specified as \codeinline{LINTER}) against all source code.
\item \codeinline{make all} or \codeinline{make} runs all of the above.
\end{itemize}
