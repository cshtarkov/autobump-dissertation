...
% example from https://lwn.net/Articles/712318/
...
manager and a function decorator. Those can be useful if using Autobump
as a library from an external application. For example, Autobump's own
unit tests override configuration options to make their success or
failure not dependent on Autobump's current configuration.
...
have a default value. Note that the \pythonline{Function}s themselves
...
\begin{tabular}{c}
\begin{python}
capir_at_commit = compose(codebase_to_units, get_commit)
\end{python}
\end{tabular}
\end{center}
