...
\centering
\includegraphics[height=0.4\textheight]{images/dot/architecture}
...
% TODO: simplejson -> Simplejson in beginning of sentence, strip textits
...
\centering
\caption{\textit{guava} statistics}
...
Only 2 out of the 3 detected mismatches are legitimate deviations from
Semver. Version \textit{2.0.0} introduces many new features compared
to \textit{1.0.0}, but no breaking changes. In version \textit{10.0.1}
some deeply nested utility methods were removed, which were
nevertheless part of the public API.

The final mismatch is in version \textit{11.0.1} where two more utility
methods were removed, but they were marked as deprecated beforehand.
Semver is ambiguous as to whether deprecated methods are part of the
public API, so this is not necessarily a breaking change.
...
% TODO: proportion of breaking vs non-breaking
