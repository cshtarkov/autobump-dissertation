...
allows for finding out exactly a piece of software is expected to
...
between divergent branches of the same initial piece of code. Note
that revision control can be applied to any type of electronic
information and not only code.
...
also change much less often and follow different policies. Popular
ones include:
\begin{enumerate}
\item Date of release. A good example is the GNU/Linux distribution
Ubuntu which follows the versioning format \textit{Year.Month} -- e.g.
Ubuntu 14.04 was released in April of 2014.
\item Arbitrary codenames, such as \textit{macOS Sierra} or
\textit{Windows Longhorn}. This scheme is typically used only with
user-facing software and in conjunction with a more traditional
sequence-based scheme.
\item Sequence-based schemes, where each version identifier is a tuple
consisting of numbers or letters -- e.g. \textit{1.3.2} or
\textit{10.6-rc1}. These are by far the most popular. It is usually
implied that the further to the left an element of the tuple is, the
more significant it is. What ``significant'' means depends on the kind
of software that is being versioned. Comparing versions \textit{5.0}
and \textit{6.0} of a Web browser may imply that the latter has new
features, whereas comparing the same versions of a library may imply
that client code using the former is incompatible with the latter. In
general, incrementing versions in this format follows these rules:
\begin{enumerate}[(i)]
...
$ \textit{3.5.4-rc2}.
\end{enumerate} In the next section, we look at an attempt to
formalize a sequence-based scheme so that the significance of the
individual elements is well-defined and conveys meaning.
\end{enumerate}
