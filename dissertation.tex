...
otherkeywords={},
keywordstyle=\ttb\color{keyword},
emph={method,MyClass,OtherClass,somePackage,YetAnotherClass,SomeClass,InnerClass},
emphstyle=\ttb\color{name},
stringstyle=\color{PineGreen},
rulecolor=\color{gray},
showstringspaces=false
...
Semver defines the public API to start at version $1.0.0$. Versions
before that impart no significance on the minor and patch numbers,
because they are expected to change radically and frequently. If the
versioning rules had started applying from the first ever release of
the project, either the developers would be discouraged from
versioning unstable work or version numbers would balloon.
...
like ones found in package managers. Applying semantic versioning
increases confidence when specifying version bounds on a requirement.
A developer can specify that their application relies on version
\textit{2.4} of a particular library. Semver guarantees that versions
of the form \textit{(2.x | x > 4)} will contain all features found in
\textit{2.4} and be backwards-compatible. If later on \textit{2.4.1}
or \textit{2.5} come along, the library can be automatically upgraded
without breaking the application, with the added benefit of fixing
bugs.
...
\section{JDiff}
% TODO: Jdiff
% http://javadiff.cvs.sourceforge.net/viewvc/javadiff/jdiff/doc/jdiff.html
...
\begin{itemize}
\item Versions all have exactly three parts: \textttd{MAJOR.MINOR.PATCH}
\item All packages start with initial version $1.0.0$
\item Versions are incremented based on how the API changes:
\begin{itemize}
\item \textttd{PATCH} - the API is the same, no risk of breaking code
\item \textttd{MINOR} - values have been added, existing values are unchanged
\item \textttd{MAJOR} - existing values have been changed or removed
\end{itemize}
\end{itemize}

The most notable change from semantic versioning is that elm-package
disallows versions smaller than $1.0.0$. Semver claims that $1.0.0$
...
version of the package is $0.9.0$ and the new changeset is breaking,
elm-package would have no way of knowing whether the next one should
be $0.9.1$, $0.10.0$ or $1.0.0$.
...
Much like Endjin.Assembly.ChangeDetection and elm-package, Autobump
looks at how the code itself has changed to estimate what the next
version should be. This requires little to no manual intervention from
the user, no special naming schemes, and is as reliable as this kind
of software can be. In contrast to these tools, we aim to make
Autobump much more general and not tied to one ecosystem or language.
For this reason, we look at how we can abstractly define breaking
changes and feature additions, and how we can apply those definitions
...
last version as breaking. By ``incompatible'', we mean changes that
make the new code technically incompatible with the old, i.e. it
doesn't compile in the case of static languages, or throws unexpected
runtime errors with dynamic languages. It does not take into
...
possibly known reliably. It also does not subjectively impart more
significance on one change than another if both are of the same class,
as we speculate humans might tend to do.
...
the minor version number. This is the approach that
...
While Autobump is guaranteed to miss some of the cases where a feature
was added because it had no bearing on the API, this does not devalue
its usefulness. The main point of semantic versioning is to guard
against breaking changes, which can be done deterministically by just
looking at the API.
...
create a tool general enough to work with multiple languages and
provide convenient ways of adding support for new ones on top of an
existing framework.
% TODO: spellcheck
...
When a configurable value of the form \textttd{Component/Option} is
requested, the module tries to fetch it from three sources and stops
at the first that succeeds:

\begin{enumerate}[(i)]
\item An environment variable named \textttd{AB_OPTION}. Note that the
...
options.
\item A configuration file \textttd{autobump.ini} present in the
working directory, which has a section \textttd{[Component]} with a
setting of the form \textttd{[option=...]} nested inside. This matches
the informal standard INI format for configuration files. The reason
we prefer this format is that Python comes with the module
...
% TODO: talk about fetching values, decorators, context managers
...
we only concern ourselves with Python 3 in this project. Moreover,
...
% TODO: explain how arrays are handled
...
reference type \javainline{OtherClass}, because we can't qualify it
before having visited all other files. If it is indeed defined in the
same package, we also have the opportunity to check whether its
descendants include \javainline{YetAnotherClass} and identify whether
this particular change is going to be breaking. Note that even if we
were aware that \javainline{OtherClass} was declared in the same
package and we could fully qualify it on the first pass, it is still
...
previous two languages we considered:
...
The Clojure handler follows a model very similar to \textttd{java_native}
...
We have compounded several reasons why relying on the AST alone is not
...
trickery, and we cannot claim for certain what a form does without
evaluating it.
\item On the same note, we cannot properly handle optional static
typing, because it relies on macro trickery.
\item We cannot check types against external libraries. If the type
hint of a parameter was changed, there's no way to verify whether the
new one is compatible with the old one.
\end{itemize}

% TODO: how we can modify describe-files
% TODO: now hooking into environment
% ... it is a typical Clojure workflow that a REPL is started and
% blabla is examined...
...
after the suite. For example, the \textttd{java_maven} test, which
goes through the history of a Maven project by using the native Java
handler, defines those two functions so that the \textttd{CLASSPATH}
is properly set up before running Autobump. In normal operation, the
\textttd{CLASSPATH} would be set up in Autobump's configuration file
...
of looking at only the public API is beneficial.
