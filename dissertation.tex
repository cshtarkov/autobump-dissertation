...
\DeclareFixedFont{\ttbt}{T1}{txtt}{bx}{n}{6}
\DeclareFixedFont{\ttmt}{T1}{txtt}{m}{n}{6}
...
void(int a)} is considered to be \javainline{(void, int)} instead of
just \javainline{(int)}. The return type of the function in CAPIR is
set to some dummy type that is compatible with everything, so that the
...
\begin{figure}[H]
\centering
\includegraphics[height=0.4\textheight]{images/dot/xmlcapir}
...
\begin{figure}[H]
\caption{Clojure source and the resultant CAPIR in intermediary S-expressions}
...
and then runs Autobump against every pair of consecutive revisions.
...
needs to say which handler should be used when running Autobump.
Tests can also optionally define \pythoninline{before_advice} and
\pythoninline{after_advice} functions, borrowind the concept of an
\textit{advice} from aspect-oriented programming, that run
respectively before and after the test. For example, the
\textttd{java_maven} test, which goes through the history of a Maven
project by using the native Java handler, defines those two functions
so that the \textttd{CLASSPATH} is properly set up before running
Autobump. In normal operation, the \textttd{CLASSPATH} would be set up
in Autobump's configuration file or by setting the environment
variable, but that is not an option when automatically running
